...
# ./fq_to_vcf_full.sh --reads /workdir/irk9/data/phg-cassava/test2/5.07x  --coordinate_ref /workdir/smm477/phg-cassava/ref/Mesculenta_671_v8.0.fa --out /workdir/irk9/data/phg-cassava/test2/5.07x/target  --threads 15

# ./fq_to_vcf_full.sh --reads /workdir/irk9/data/phg-maize/test2/5.07x  --coordinate_ref /workdir/irk9/data/phg-maize/trial/B73.fa --out /workdir/irk9/data/phg-maize/test2/5.07x/target  --threads 15

# ./fq_to_vcf_full.sh --reads /workdir/irk9/data/phg-cassava/test2_redo/26.35x  --coordinate_ref /workdir/smm477/phg-cassava/ref/Mesculenta_671_v8.0.fa --out /workdir/irk9/data/phg-cassava/test2_redo/26.35x/target --threads 14
...
if [[ -z "$READS_DIR" || -z "$REF_MAYBE_DIR" || -z "$OUT_DIR" ]]; then
...
for reads in "$READS_DIR"/*.fastq.gz
do
    [ -e "$reads" ] || continue
    ...
    if bwa mem -t "$THREADS" "$REF_MAYBE_DIR" "$reads" 2>> "$log_file" \
