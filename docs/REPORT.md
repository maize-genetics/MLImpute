...
model, and the experiment series E1–E12. Full per-experiment detail is in
...
### E11 — Diploid pair-state CRF on a breeding-population panel

The diploid model decodes over **P=325 founder pairs** (`emis_p = emis[i]+emis[j]`,
same encoder, zero added params), evaluated on a structured breeding-population sim
(k2-F2, k8-S1, and outbred cells × inbred/het). The genome-wide **founder affinity**
prior (E5, here fed as an encoder conditioning signal rather than a hard cutoff) helps
**every cell**; the best-val checkpoint `e11-breedpop-affinity` reaches **0.7587**
pair-accuracy. Against a Li–Stephens HMM on the same data the learned emission +
transition stack wins **3.7×** (0.76 vs 0.21) — the HMM's homozygous-collapse on
single-read diploid emissions tanks it. Training is single-epoch best-val
checkpointed (the epoch-1 collapse is overfit-on-repeat, not a gradient spike).

### E12 — Whole-genome scale: edge-free decode, a smaller CRF, and the imputation metric

![wholegenome](figures/fig9_e12_wholegenome.png)

The encoder is capped at ~1024-site context, but the pair-state CRF decode is **linear**,
so it spans a whole chromosome: overlapped encoder windows, emissions stitched by
center-crop, then **one Viterbi over the full 100k-site chromosome** — removing the
per-tile decode-boundary errors. On 48 whole-genome individuals (6 cells × 4 ×
{sparse, dense}) the whole-chrom decode matches E11 (dense ALL 0.795 pair).

**The imputation metric.** Founder-path accuracy asks *did we name the right founder*;
an imputation user wants *did we recover the right allele*. We measure it label-free by
**masking 1% of whole sites**, decoding, and scoring **either-match** at the held-out
sites (correct if *either* decoded founder reproduces the hidden read). It needs no
founder truth, so the same call runs on real maize/cassava. The result: on dense panels
**SNP accuracy (0.93–0.96) sits far above founder-ID** — the gap is the
*allele-redundancy cushion* (outbred-het 0.55→0.88, k8-het 0.73→0.93): founder mistakes
that pick an allele-identical founder cost nothing. On sparse panels founder-ID is
already ~0.99 and SNP saturates the ~0.96 bad-frac ceiling.

![prune](figures/fig10_e12_prune.png)

**A smaller CRF from affinity.** If affinity says which founders an individual carries,
decode only that pair-state subset. A sorted-rate largest-gap split recovers the carried
set and cuts **P 325→6 (k2-F2) or →45 (k8-S1)**. GPU Viterbi is launch-bound (P barely
moves wall-time), but CPU is compute-bound, so pruning gives a **~10× CPU decode speedup**
(and pruned-CPU beats full-GPU ~3×). It is **safe on dense panels** (≈ +0.005, helps k8 by
dropping spurious background states) but **unsafe on sparse** ones: a sparsely-carried
founder's affinity falls into the background and the split drops it, collapsing accuracy
(k8-het 0.99→0.31). So pruning stays **opt-in, default off** — and the label-free
masked-SNP metric independently flags the failure, doubling as a real-data safety gate.
...
5. The diploid pair-state CRF scales to **whole genomes** with an edge-free
   chromosome-long decode, and the affinity prior lifts every breeding-population
   cell (E11–E12).
6. At the SNP level the **imputation accuracy is far higher than the founder-path
   accuracy** (E12 masked-site metric) — and it is **label-free**, so it is the
   bridge to scoring on real data where founder truth is unavailable.
...
is not needed), a **GPU Viterbi T-loop that batches launches** (E12 — the remaining
decode lever, orthogonal to pruning), and promotion to **real diploid maize/cassava**
(scored via the label-free masked-site metric).
