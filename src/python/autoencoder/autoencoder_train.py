...
def evaluate_model(model, test_loader, test_matrix, window_size, step_size, device, batch_size):
    ...
    with torch.no_grad():
        for batch_idx, (batch_data, decode_dict) in enumerate(test_loader):
            ...
            loss = model.compute_loss(outputs, batch_data)
...
def train_model(model, train_loader, optimizer, epochs, device, test_loader, test_matrix, window_size, step_size,
    ...
    wandb.init(project="AutoEncoder Imputation", name="autoencoder-imputation", config={
        "epochs": epochs,
...
# ✅ Main Function
def main():
    wandb.init(project="AutoEncoder Imputation", name="autoencoder", entity="maize-genetics")
    ...
    model = AutoEncoder(num_parents=num_classes, window_size=window_size, hidden_dim=hidden_dim, bottleneck_dim=bottleneck_dim, dropout=dropout)
