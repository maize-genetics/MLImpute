...
def run_refmap(sample, reads_fastq, outdir, arm="refmap"):
...
def load_contig_layout(bins_path, window_size=WINDOW_SIZE):
    ...
    for contig, idx in bins_df.groupby("contig", sort=False).indices.items():
        ...
        positions = bins_df.loc[used, "bin"].to_numpy() * 256  # refPosBinned -> real bp
...
def write_imputed_bed(sample, pi_arr, pj_arr, dropped_idx, gamete_names, bins_path, bed_dir):
    ...
    layout = load_contig_layout(bins_path, T)
