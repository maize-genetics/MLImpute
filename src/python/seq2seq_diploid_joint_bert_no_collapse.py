...
class PreWindowedHaploidDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels = row[:, self.K:(self.K+2)].astype(np.int64)
...
class EncoderDiploid(nn.Module):
    def __init__(self, in_features, emb_dim, window_size):
        super().__init__()
    ...
    def forward(self, src_feats):
        # src_feats: (seq_len, batch, in_features)
        x = src_feats.permute(1, 0, 2)   # (batch, seq_len, in_features)
        emb = self.proj(x)
...
class Seq2SeqDiploidJoint(nn.Module):
    def __init__(self, encoder, decoder, device, ploidy=2):
        ...
        self.device = device
        self.ploidy = ploidy
        self.enc_to_dec = nn.Linear(encoder.emb_dim, decoder.hidden_dim)
    ...
    def forward(self, src_feats, trg=None, teacher_forcing_ratio=0.5):
        ...
        # Encoder
        encoder_outputs = self.encoder(src_feats)  # (batch, seq_len, emb_dim)

        hidden = encoder_outputs.mean(dim=1)
        hidden = self.enc_to_dec(hidden).unsqueeze(0)
        ...
        for t in range(seq_len):
            context = encoder_outputs[:, t, :]  # (batch, emb_dim)
...
# run model using saved checkpoint
def runSeq2SeqDiploidJoint(args, data, weights):
    ...
    enc = EncoderDiploid(num_parents, EMB_DIM, max_seq_length)
    dec = DecoderDiploidJoint(num_parents + 1, EMB_DIM, HID_DIM, ploidy=ploidy)
    ...
    test_dataset = WindowIndexDatasetFromMatrix(data, weights, window_size=window_size, top_n=num_parents,
                                               step_size=window_size, return_decode=True)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=0)

    # Seq2SeqDiploidJoint outputs (B, T, V, V) joint pair logits — use decode_joint_predictions
    final_predictions = []
    with torch.no_grad():
        for batch_data, decode_dict in tqdm(test_loader):
            batch_data = batch_data.to(device)
            decode_dict = decode_dict.to(device)

            logits = model(batch_data)                      # (B, T, V, V)
            pred_pairs = decode_joint_predictions(logits)   # (B, T, 2)
            

            B, T, _ = pred_pairs.shape
            row_ids = torch.arange(B, device=device).unsqueeze(1).expand(B, T)
            # clamp in case unknown token (index num_parents) is predicted
            p1_idx = pred_pairs[..., 0].clamp(0, num_parents - 1)
            p2_idx = pred_pairs[..., 1].clamp(0, num_parents - 1)
            p1 = decode_dict[row_ids, p1_idx]  # (B, T) actual parent IDs
            p2 = decode_dict[row_ids, p2_idx]
            

            batch_preds = [
                [tuple(pair.tolist()) for pair in sample]
                for sample in torch.stack([p1, p2], dim=-1)
            ]
            final_predictions.extend(batch_preds)

    final_predictions = [tup for batch in final_predictions for tup in batch]
    
    return np.array(final_predictions, dtype=np.int16)
...
def parse_args():
    ...
    parser.add_argument("--step-size", "-s", type=int, default=512, help="distance between the start points of each training window")
...
def main():
    ...
    enc = EncoderDiploid(args.num_parents, EMB_DIM, args.max_seq_length)
    ...
    for epoch in range(args.num_epochs):
        ...
        train_loss, train_acc = train(model, train_loader, optimizer, criterion, args.steps_to_print)
