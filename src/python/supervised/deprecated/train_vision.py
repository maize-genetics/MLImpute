...
from dataset_labeled import VisionSegmentationDataset
from loss_functions import BinomialKLLoss, ViTCrossEntropy
import sys
...
def parse_args():
    ...
    parser.add_argument("--loss-type", type=str, default="mean", help="type of categorical cross entropy loss to use. Choose from 'mean', 'sum' or 'distributed")
...
def main():
    ...
    if args.loss_type == "sum":
        criterion = ViTCrossEntropy(reduction="sum")
    elif args.loss_type == "fuzzy":
        criterion = BinomialKLLoss(reduction="sum")
    else:
        criterion = ViTCrossEntropy(reduction="mean")
    ...
    dataset = VisionSegmentationDataset(args.keyfile, windows=args.windows)
