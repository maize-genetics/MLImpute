...
data = np.load(f"diploid-data/test/{sample}_chr{chr}_matrix.npy", allow_pickle=True)
print(data.shape)
...
all_predictions = np.load(f"model_preds/diploid/{sample}_chr{chr}.npy", allow_pickle=True).astype(float)
