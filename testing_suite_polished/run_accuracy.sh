...
# ./run_accuracy.sh --base-dir /workdir/irk9/data/phg-cassava --target-dir /workdir/irk9/data/phg-cassava/test3 --truth-vcf /workdir/irk9/data/phg-cassava/truth-vcfs/cassava_pangenome_diploid_rename_chr.vcf.gz --script-dir /workdir/irk9/data/phg-maize/bellas_scripts  --out-dir /workdir/irk9/data/phg-cassava/test3/eval_snp --species cassava --imputers grits --map-flags nomap --coverages 0.01,0.1,1,2 --samples-list /workdir/irk9/data/phg-cassava/target_vcf/cassava_grits_samples.txt


# ./run_accuracy.sh --base-dir /workdir/irk9/data/phg-maize/ --target-dir /workdir/irk9/data/phg-maize/test3 --truth-vcf /workdir/smm477/evaluate/truth-vcfs/maize_pangenome_snps.vcf.gz --script-dir /workdir/irk9/data/phg-maize/bellas_scripts  --out-dir /workdir/irk9/data/phg-maize/test3/eval_snp --species maize --imputers impute_haploid --map-flags nomap --coverages 0.01,0.1,1,2 --samples-list /workdir/irk9/data/phg-maize/target_vcf/samples.txt


# ./run_accuracy.sh --base-dir /workdir/irk9/data/phg-cassava --target-dir /workdir/irk9/data/phg-cassava/test2_redo --truth-vcf /workdir/shared_files/cassava_test2_answer_key_redo/cassava_test2_answer_key_snps.vcf.gz --script-dir /workdir/irk9/data/phg-maize/bellas_scripts  --out-dir /workdir/irk9/data/phg-cassava/test2_redo/eval_snp --species cassava --imputers impute,minimac,beagle,grits --map-flags nomap --coverages 26.35 --samples-list /workdir/irk9/data/phg-cassava/eval/test2_samples_list.txt
...
clean_grits() {
    ...
    bcftools annotate -h "${contig_hdr}" -Ov -o "${tmp_vcf}" "${src_vcf}" \
        || { echo "[WARN] pre-annotate failed for grits ${sample}"; rm -f "${tmp_vcf}" "${contig_hdr}" "${chr_rename}"; return 0; }
    ...
    rm -f "${work_dir}/_sname.txt" "${contig_hdr}" "${chr_rename}" "${tmp_vcf}"
    # "${chr_rename}" "${contig_hdr}"
